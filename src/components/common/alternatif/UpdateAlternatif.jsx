...
import {
  changeNameAlternatif,
  changeNilaiKriteria,
  getAllDataEditAlternatif,
} from '../../../store/alternatif/UpdateAlternatif';
import { updateAlternatif } from '../../../config/Alternatif';

const UpdateAlternatif = ({
  openUpdateAlternatif,
  setOpenUpdateAlternatif,
}) => {
  ...
  const saveData = async (kode, kodeKriteria, nilai) => {
    ...
    try {
      await Promise.all(
        nilai.map((_, idx) =>
          updateAlternatif(kode, kodeKriteria[idx], { nilai: nilai[idx] })
        )
      );
    } catch (err) {
      console.error(err);
    ...
    <Dialog open={openUpdateAlternatif} fullWidth>
      ...
      <DialogContent>
        <Grid container columns={12} spacing={2}>
          <Grid item xs={12}>
            <CustomInput value={kode} disabled label={'Alternative code'} />
          </Grid>
